...
最后更新：2026-09-14 18:15:03

- [x] 已完成并验证：将单 CSV 成功删行改为启动时加载内存并每 1000 条原子批量写回 - 2026-09-14 18:15:03

- [x] 已完成并验证：仅在成功页面保留TCP结束挥手等待，失败页面立即停止抓包 - 2026-09-14 17:54:39

- [x] 已完成并验证：将 windows01 风格的非无痕 Chrome 参数并入 `chrome.py`，恢复单 CSV 默认后端并删除临时变体文件 - 2026-09-14 17:26:08

- [x] 已完成并验证：修复单 CSV `CONTAINER_COUNT` 被误当固定并发数，将其统一解释为动态容器数上限 - 2026-09-12 21:49:16

- [x] 已完成并验证：统一 Chrome 无头采集的 4K 虚拟视口与设备像素比，提升响应式资源分辨率 - 2026-09-11 19:36:03

- [x] 已完成并验证：新增 The Guardian 分类文件整合脚本，使用 `Section_ID` 生成 `id` - 2026-09-11 16:56:06
- [x] 已完成并验证：新增 The Guardian TSV 转换脚本并生成 `theguardian_sensitive1_urls.csv` - 2026-09-11 16:29:50
- [x] 已完成并验证：新增 CSV URL 编码脚本，默认备份后原地编码 `wiki5.csv` - 2026-09-09 19:08:32
- [x] 已完成并验证：将出口 IP 检测地址替换为 AWS、IPinfo 和 ident.me 公网服务 - 2026-09-09 16:57:34
- [x] 已完成并验证：让 Chrome 镜像升级脚本从 `BaseTrafficIngestor.DOCKER_IMAGE` 获取默认源镜像，并将项目默认镜像更新为 `151_260909` - 2026-09-09 15:23:47
- [x] 已完成并验证：让 Chrome 镜像升级每次强制刷新 APT 索引并输出 Chrome 候选版本 - 2026-09-09 15:33:03
- [x] 已完成并验证：仅在 Chrome 主版本高于现有镜像时推送升级镜像 - 2026-09-09 15:42:58

- [x] 已完成并验证：新建 `docs/design/VPN与TLS流量采集流程对比.md`，记录普通 TLS 与 Clash/Trojan VPN 采集流程、TLS 分层、keylog、SNI 和 Wireshark 处理边界 - 2026-09-08 10:25:30

- [x] 已完成并验证：调整出口 IP 探测脚本为无代码挂载、无 offload 操作且不执行 CSV 采集 - 2026-09-08 09:16:09
- [x] 已完成并验证：调整出口 IP 探测脚本，将运行参数统一改为全局变量并验证远程运行 - 2026-09-07 22:02:13
- [x] 已完成并验证：新增 `scripts/check_container_exit_ips.py`，按指定单 CSV profile 启动指定数量 Docker 并查询各容器公网出口 IPv4 - 2026-09-07 21:45:48

- [x] 已完成并验证：将 `bbc_sensitive.csv` 和 `bbc_white.csv` 转换为 `id,url,domain` 格式并输出到 `scripts/result/`；跳过白名单末尾缺少 URL 的截断记录 `10001` - 2026-08-21 15:22:14

- [x] 已完成并验证：将 `voa_sensitive.csv` 和 `voa_white.csv` 转换为 `id,url,domain` 格式并输出到 `scripts/result/` - 2026-08-21 15:13:43

- [x] 已完成并验证：在远程服务器启动 FRA、SGP 各 2 个及 US 默认 bridge 15 个测试容器并核验公网出口 IPv4 - 2026-08-17 17:37:24

- [x] 已完成并验证：补充与具体文件解绑且至少容纳 1000 个容器的 FRA、SGP 默认 Docker 网络参数约束 - 2026-08-17 17:28:29

- [x] 已完成并验证：将页面正文文件后缀从 `.text` 统一改为 `.txt` - 2026-08-17 16:35:51

- [x] 已完成并验证：收紧人机验证检测，禁止仅凭标题中的“验证码”等词判定验证页 - 2026-08-13 17:11:32

- [x] 已完成并验证：将项目默认 Chrome 镜像切换到 `chuanzhoupan/trace_spider_chrome:151_260811` - 2026-08-11 19:58:44

- [x] 已完成并验证：修复 Chrome 镜像升级工具的旧源密钥、APT 超时及 ChromeDriver 兼容版本匹配问题，并推送 Chrome 151 镜像 - 2026-08-11 19:54:16

- [x] 已完成并验证：在远程服务器建立 E1 SGP 连续18个容器并核验实际公网出口 IP - 2026-07-31 00:06:01

- [x] 已完成并验证：将 E1 SGP 扩容为 `172.18.4.0/22` 专属网络的 1000 容器配置 - 2026-07-30 23:58:19

- [x] 已完成并验证：在远程服务器使用 E1 SGP 的 18 个容器和 E2 FRA 的 11 个容器验证出口 IP - 2026-07-24 15:42:12

- [x] 已完成并验证：在远程服务器为 E0 US、E1 SGP、E2 FRA 各建立 2 个容器并验证出口 IP - 2026-07-24 15:35:57

- [x] 已完成并验证：让单 CSV 运行名称默认按配置文件名生成并支持显式覆盖 - 2026-07-24 15:18:27

- [x] 已完成并验证：为单 CSV 页面采集增加 HTTP 失败码与人机验证校验 - 2026-07-23 14:41:19

- [x] 已完成并验证：修复多 CSV 调度器首批任务缺少任务序号初始化 - 2026-07-22 09:04:01

- [x] 已完成并验证：修复多 CSV 入口日志接口并按 CSV 实际记录数计算采集目标 - 2026-07-22 00:57:39

- [x] 已完成并验证：为多 CSV 目录采集入口增加可覆盖镜像配置 - 2026-07-22 00:35:15

- [x] 已完成并验证：将单 CSV 执行日志改为控制台文本输出 - 2026-07-20 17:31:53

- [x] 已完成并验证：清理 HOST_CODE_PATH 时跳过只读 Clash 挂载点 - 2026-07-20 17:20:17

- [x] 已完成并验证：修复只读挂载的 Clash core 缺少执行权限导致启动失败 - 2026-07-20 16:53:32

- [x] 已完成并验证：将 Clash 程序改为只读共享挂载并隔离每个容器的配置与日志 - 2026-07-20 16:40:30

- [x] 已完成并验证：统一 Clash 与 HTTPS 单 CSV 配置文件加载方式并补充启动命令 - 2026-07-20 15:50:09

- [x] 已完成并验证：为 GitHub 仓库 CSV 转换脚本增加重复次数参数 - 2026-07-20 15:21:24

- [x] 已完成并验证：在远程 tmux 验证 Firefox ECH 动态加载、密钥标签输出与 QUIC 禁用 - 2026-07-15 20:12:51

- [x] 已完成并验证：取消浏览器路径中央注册并接入 Firefox ECH 密钥标签输出校验 - 2026-07-15 20:04:55

- [x] 已完成并验证：让 Firefox ECH 变体同时强制 ECH 并禁用 QUIC - 2026-07-15 19:42:41

- [x] 已完成并验证：将 Firefox 采集镜像统一更新为 152.0.6 - 2026-07-15 19:40:26

- [x] 已完成并验证：增加成功后是否删除 CSV 记录的配置开关 - 2026-07-15 19:01:57

- [x] 已完成并验证：将重复的 trafficIngestor/trafficIngestor 包更名为 host_scheduler - 2026-07-15 18:54:18

- [x] 已完成并验证：在非 Clash 单 CSV 入口顶部补充使用方法注释 - 2026-07-15 18:44:11

- [x] 已完成并验证：将 ACTION_PROFILE 改为明确的浏览器 Python 文件路径 - 2026-07-15 18:24:47

- [x] 已完成并验证：固定单 CSV 公共配置与运行策略，并将浏览器实现归入独立目录 - 2026-07-15 17:50:08

- [x] 已完成并验证：适配源码根目录由 src 更名为 trafficIngestor，并接入新的 single_csv 配置路径 - 2026-07-15 17:03:09

- [x] 已完成并验证：删除根目录仅剩受保护 `.git` 的 `clash-for-linux/` 残留目录 - 2026-07-15 17:03:09
