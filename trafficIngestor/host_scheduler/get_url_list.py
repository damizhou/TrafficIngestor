...
HOST_CODE_PATH = os.path.join(_source_root, BASE_NAME)
...
SOURCE_CSV = os.path.join(_project_root, "scripts", "top4000.csv")
...
def run_once(names: List[str], jobs: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    with tqdm(
        total=len(jobs),
        desc=f"任务进度: 0/{len(jobs)}个 [剩余: {len(jobs)} | 预计剩余: 计算中 | 初始化中...]",
        leave=True,
    ) as pbar:
        with _pbar_lock:
            _pbar = pbar
