...
class BaseClashTrafficIngestor(BaseTrafficIngestor):
    ...
    def get_vpn_info_name(self) -> str:
        """Return the node-list variable name in configs/clash/sever_info.py."""
    ...
    def get_vpn_items(self) -> List[Dict[str, Any]]:
        ...
        sever_info = importlib.import_module("configs.clash.sever_info")
        if not hasattr(sever_info, vpn_info_name):
            raise RuntimeError(
                f"configs/clash/sever_info.py 中找不到节点数组 {vpn_info_name}"
            )
    ...
    def _load_vpns(self) -> List[Dict[str, Any]]:
        ...
        if not isinstance(raw_vpns, list) or not raw_vpns:
            raise RuntimeError(
                f"configs/clash/sever_info.py 中的 {vpn_info_name} 不能为空"
            )
