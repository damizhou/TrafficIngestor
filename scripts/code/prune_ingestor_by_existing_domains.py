...
- 若某条记录的 domain 在目标目录存在同名文件夹，则删除该条记录。

使用方式:
- 预览删除: python scripts/code/prune_ingestor_by_existing_domains.py --dry-run
- 原地覆盖: python scripts/code/prune_ingestor_by_existing_domains.py
- 输出到新文件: python scripts/code/prune_ingestor_by_existing_domains.py --output top2000_ingestor.filtered.csv
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Remove rows when row domain exists as a folder under target directory. "
...
def main() -> int:
    ...
    try:
        ...
        fieldnames, kept_rows, total, removed = filter_rows(input_csv, existing_domains)
