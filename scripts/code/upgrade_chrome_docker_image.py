...
DEFAULT_SOURCE_IMAGE = "chuanzhoupan/trace_spider_chrome_149:260611"
...
DOCKERFILE = r"""ARG SOURCE_IMAGE=chuanzhoupan/trace_spider_chrome_149:260611
FROM ${SOURCE_IMAGE}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "升级基础镜像中的 Chrome 与 ChromeDriver，验证版本一致后推送新镜像。"
        )
    )
    parser.add_argument(
        "source_image",
        ...
        help=f"基础镜像，默认：{DEFAULT_SOURCE_IMAGE}",
    )
    parser.add_argument(
        "target_namespace",
        nargs="?",
