...
```text
src/                                项目源码根目录
src/trafficIngestor/                非 Clash 宿主机调度与单 CSV 入口
src/trafficIngestor_clash/          Clash 宿主机调度入口
src/traffic_capture_single_csv/     公共浏览器容器执行入口
src/traffic_capture_single_db/      数据库容器执行入口
src/multi_csv_traffic_ingestor/     多 CSV 调度入口
src/url_list_collector/             Scrapy URL 收集子项目
src/tools/                          公共浏览器、抓包、日志与 Action 基类
configs/single_csv/                 非 Clash 单 CSV 独立配置目录
...
非 Clash 单 CSV 采集器的配置位于 `configs/single_csv/`，一个 `.py` 文件对应一种配置；`src/trafficIngestor/single_csv_profiles.py` 只负责加载命令行指定的配置文件并启动任务。Clash 配置暂时仍在 `src/trafficIngestor_clash/single_csv_profiles.py`。默认 Docker 镜像由 `BaseTrafficIngestor.DOCKER_IMAGE` 提供，Edge / Firefox、固定 IP 和 Clash 等特殊配置会显式覆盖所需字段。

### 2. 修改配置并直接运行

非 Clash 任务必须显式传入配置文件路径。修改 `configs/single_csv/base.py` 中的 `CONFIG`、`RUN_POLICY`、`RUNTIME_NAME` 或 `ACTION_PROFILE` 后执行：

```powershell
python src/trafficIngestor/single_csv_profiles.py configs/single_csv/base.py
```

Clash 任务仍从其入口文件中的 profile 配置启动：

```powershell
python src/trafficIngestor_clash/single_csv_profiles.py clash
```
...
- `python src/trafficIngestor/single_csv_profiles.py configs/single_csv/base.py`
  Chrome 批量流量采集
- `python src/trafficIngestor/single_csv_profiles.py configs/single_csv/fixed_ip_europe.py`
  Chrome 批量流量采集，容器挂到独立网络 `traffic_ingestor_fixed_ip_europe_net`，IP 从 `172.18.0.2` 开始递增
- `python src/trafficIngestor/single_csv_profiles.py configs/single_csv/fixed_ip_rsia.py`
...
- `python src/trafficIngestor_clash/single_csv_profiles.py fixed_ip_europe`
  Chrome + Clash 欧洲入口，使用 `configs/clash/sever_info.py` 中的 `vpns_info_europ` 节点数组；网络隔离逻辑与普通 Clash 入口一致
- `python src/trafficIngestor/single_csv_profiles.py configs/single_csv/edge.py`
  Edge 测试或定向采集
- `python src/trafficIngestor/single_csv_profiles.py configs/single_csv/firefox.py`
  Firefox 测试或定向采集
- `python src/trafficIngestor/traffic_capture_single_db.py`
  从数据库读取新闻 URL 批量采集
- `python src/trafficIngestor/get_url_list.py`
  批量采集站点子页面 URL
...
- `configs/single_csv/`：15 个非 Clash 配置，一个配置一个文件
- `src/trafficIngestor/single_csv_profiles.py`：非 Clash 指定配置文件的加载、校验与运行入口
- `src/trafficIngestor_clash/single_csv_profiles.py`：7 个 Clash profile

公共任务源和运行策略位于 `src/trafficIngestor/csv_ingestor_common.py`。非 Clash 配置文件必须定义 `CONFIG`、`RUN_POLICY`、`RUNTIME_NAME` 和 `ACTION_PROFILE`，运行时必须显式传入该 `.py` 文件的路径。每个配置显式保存运行命名空间，动态工作目录统一写入 `runtime/workspaces/`。

Chrome、Edge、Firefox、Firefox Disable 及其 Clash 变体统一使用 `src/traffic_capture_single_csv/action.py`。宿主 profile 通过 `TRAFFIC_ACTION_PROFILE` 向容器传入浏览器后端和代理模式；运行前会把公共 action 同步到 `runtime/workspaces/` 中该 profile 的独立目录。
...
`src/trafficIngestor_clash/single_csv_profiles.py` 中的 profile 额外启用了“运行命名空间”隔离：每个 profile 显式保存原入口名称，并据此生成 `BASE_NAME`、`HOST_CODE_PATH`、`CONTAINER_PREFIX` 和 `DOCKER_NETWORK`。基类不会创建整个 `172.19.0.0/16`，而是把它当作地址池，按顺序扫描可用的 `/22` 子网并依次使用 `172.19.0.0/22`、`172.19.4.0/22`、`172.19.8.0/22`……；新建的自动子网默认使用 `.1` 作为网关、`.2` 作为首个容器 IP。建议在宿主机代理或 `v2raya` 配置中将 `172.19.0.0/16` 整段设为直连。若需要显式覆盖命名空间，可设置环境变量 `TRAFFIC_INGESTOR_RUN_NAME`。
...
```powershell
python -m py_compile src\tools\base_action.py src\tools\chrome.py src\tools\edge.py src\tools\firefox.py src\traffic_capture_single_csv\action.py
```
...
- 浏览器路径和驱动路径主要写死在 `src/tools/chrome.py`、`src/tools/edge.py`、`src/tools/firefox.py`，更换镜像时要同步检查。
...
- `python src/trafficIngestor_clash/single_csv_profiles.py edge`
  Edge + Clash 采集；使用 Edge 镜像、Edge 版本探测和原 Edge Clash 运行命名空间。
- `python src/trafficIngestor_clash/single_csv_profiles.py firefox`
...
- `TRAFFIC_INGESTOR_RUN_NAME=my_firefox_batch python src/trafficIngestor_clash/single_csv_profiles.py firefox`
...
- `src/trafficIngestor_clash/` 基类现在会在容器启动 Clash 时导出 `SSLKEYLOGFILE`，默认路径为 `HOST_CODE_PATH/clash_runtime/<container>/trojan_outer_sslkey.log`。
