...
trafficIngestor/single_csv_profiles.py
                                    非 Clash 单 CSV 配置加载与运行入口
single_csv_configs/                 非 Clash 单 CSV 独立配置目录
single_csv_configs/base.py          默认 Chrome 配置；一个配置一个文件
trafficIngestor_clash/single_csv_profiles.py
                                    Clash 单 CSV 直接运行入口与配置字典
...
small_tools/                        输入 CSV、测试数据与数据维护工具
db/                                 数据库配置
...
非 Clash 单 CSV 采集器的配置位于根目录 `single_csv_configs/`，一个 `.py` 文件对应一种配置；`trafficIngestor/single_csv_profiles.py` 只负责加载命令行指定的配置文件并启动任务。Clash 配置暂时仍在 `trafficIngestor_clash/single_csv_profiles.py`。默认 Docker 镜像由 `BaseTrafficIngestor.DOCKER_IMAGE` 提供，Edge / Firefox、固定 IP 和 Clash 等特殊配置会显式覆盖所需字段。

### 2. 修改配置并直接运行

非 Clash 任务必须显式传入配置文件路径。修改 `single_csv_configs/base.py` 中的 `CONFIG`、`RUN_POLICY`、`RUNTIME_NAME` 或 `ACTION_PROFILE` 后执行：

```powershell
python trafficIngestor/single_csv_profiles.py single_csv_configs/base.py
```

Clash 任务同理，修改 `trafficIngestor_clash/single_csv_profiles.py` 文件顶部的 `CONFIG = {...}` 后执行：

```powershell
python trafficIngestor_clash/single_csv_profiles.py
```
...
- `python trafficIngestor/single_csv_profiles.py single_csv_configs/base.py`
  Chrome 批量流量采集
- `python trafficIngestor/single_csv_profiles.py single_csv_configs/fixed_ip_europe.py`
  Chrome 批量流量采集，容器挂到独立网络 `traffic_ingestor_fixed_ip_europe_net`，IP 从 `172.18.0.2` 开始递增
- `python trafficIngestor/single_csv_profiles.py single_csv_configs/fixed_ip_rsia.py`
  Chrome 批量流量采集，容器挂到独立网络 `traffic_ingestor_fixed_ip_rsia_net`，IP 从 `172.18.2.2` 开始递增
- `python -m trafficIngestor_clash.single_csv_profiles clash`
  Chrome + Clash 批量流量采集，运行时按 profile 的固定命名空间创建独立 Docker 网络，并从 `172.19.0.0/16` 地址池中选择可用 `/22` 子网
- `python -m trafficIngestor_clash.single_csv_profiles fixed_ip_europe`
  Chrome + Clash 欧洲入口，使用 `config/sever_info.py` 中的 `vpns_info_europ` 节点数组；网络隔离逻辑与普通 Clash 入口一致
- `python trafficIngestor/single_csv_profiles.py single_csv_configs/edge.py`
  Edge 测试或定向采集
- `python trafficIngestor/single_csv_profiles.py single_csv_configs/firefox.py`
  Firefox 测试或定向采集
...
- `single_csv_configs/`：15 个非 Clash 配置，一个配置一个文件
- `trafficIngestor/single_csv_profiles.py`：非 Clash 指定配置文件的加载、校验与运行入口
- `trafficIngestor_clash/single_csv_profiles.py`：7 个 Clash profile

公共任务源和运行策略位于 `trafficIngestor/csv_ingestor_common.py`。非 Clash 配置文件必须定义 `CONFIG`、`RUN_POLICY`、`RUNTIME_NAME` 和 `ACTION_PROFILE`，运行时必须显式传入该 `.py` 文件的路径。每个配置显式保存原入口的运行命名空间，因此合并后 `BASE_NAME`、`CONTAINER_PREFIX`、默认 `HOST_CODE_PATH` 和运行锁保持不变。

Chrome、Edge、Firefox、Firefox Disable 及其 Clash 变体统一使用 `traffic_capture_single_csv/action.py`。宿主 profile 通过 `TRAFFIC_ACTION_PROFILE` 向容器传入浏览器后端和代理模式；运行前会把公共 action 同步到该 profile 的独立 `HOST_CODE_PATH`，因此运行目录仍隔离，但仓库不再维护多份 action 源码。
...
`trafficIngestor_clash/single_csv_profiles.py` 中的 profile 额外启用了“运行命名空间”隔离：每个 profile 显式保存原入口名称，并据此生成 `BASE_NAME`、`HOST_CODE_PATH`、`CONTAINER_PREFIX` 和 `DOCKER_NETWORK`。基类不会创建整个 `172.19.0.0/16`，而是把它当作地址池，按顺序扫描可用的 `/22` 子网并依次使用 `172.19.0.0/22`、`172.19.4.0/22`、`172.19.8.0/22`……；新建的自动子网默认使用 `.1` 作为网关、`.2` 作为首个容器 IP。建议在宿主机代理或 `v2raya` 配置中将 `172.19.0.0/16` 整段设为直连。若需要显式覆盖命名空间，可设置环境变量 `TRAFFIC_INGESTOR_RUN_NAME`。
...
```powershell
python -m py_compile tools\base_action.py tools\chrome.py tools\edge.py tools\firefox.py traffic_capture_single_csv\action.py
```
...
- `python small_tools/code/check_pcap_dataset.py --help`：检查 pcap 数据集完整性；`--remove-unqualified` 会删除目录和 CSV 记录，使用前必须核对路径。
- `python small_tools/code/merge_csv.py --help`：按 domain 合并主 CSV 与临时 CSV，支持 `--dry-run`。
- `python small_tools/code/move_categories.py`：按 pcap 总体积保留前 N 个类别，并在确认后移动其余目录。
...
- `python -m trafficIngestor_clash.single_csv_profiles edge`
  Edge + Clash 采集；使用 Edge 镜像、Edge 版本探测和原 Edge Clash 运行命名空间。
- `python -m trafficIngestor_clash.single_csv_profiles firefox`
  Firefox + Clash 采集；使用 Firefox 镜像、Firefox 版本探测和原 Firefox Clash 运行命名空间。
- `python -m trafficIngestor_clash.single_csv_profiles fixed_ip_europe`
  Chrome + Clash 欧洲采集；profile 通过 `VPN_INFO_NAME = "vpns_info_europ"` 指定 `config/sever_info.py` 里的节点数组。
- `TRAFFIC_INGESTOR_RUN_NAME=my_firefox_batch python -m trafficIngestor_clash.single_csv_profiles firefox`
