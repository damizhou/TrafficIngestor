...
trafficIngestor/host_scheduler/     非 Clash 宿主机调度与单 CSV 入口
trafficIngestor/host_scheduler_clash/
                                    Clash 宿主机调度入口
...
trafficIngestor/single_csv/         HTTPS 与 Clash 单 CSV 独立配置目录
...
单 CSV 采集配置统一位于 `trafficIngestor/single_csv/`，一个 `.py` 文件对应一种配置。HTTPS 与 Clash 入口使用同一套配置加载和校验逻辑，分别由 `trafficIngestor/host_scheduler/single_csv_profiles.py` 与 `trafficIngestor/host_scheduler_clash/single_csv_profiles.py` 启动。默认 Docker 镜像由 `BaseTrafficIngestor.DOCKER_IMAGE` 提供，Edge、Firefox、固定节点等特殊配置会显式覆盖所需字段。
...
Clash 任务同样显式传入配置文件路径。例如 GitHub Trojan 采集执行：
...
- `python trafficIngestor/host_scheduler/single_csv_profiles.py trafficIngestor/single_csv/base.py`
  Chrome 批量流量采集
- `python trafficIngestor/host_scheduler/single_csv_profiles.py trafficIngestor/single_csv/fixed_ip_europe.py`
...
- `python trafficIngestor/host_scheduler/single_csv_profiles.py trafficIngestor/single_csv/edge.py`
  Edge 测试或定向采集
- `python trafficIngestor/host_scheduler/single_csv_profiles.py trafficIngestor/single_csv/firefox.py`
  Firefox 测试或定向采集
- `python trafficIngestor/host_scheduler/traffic_capture_single_db.py`
  从数据库读取新闻 URL 批量采集
...
- `trafficIngestor/single_csv/`：HTTPS 与 Clash 共用的独立配置文件目录
...
`ACTION_PROFILE` 直接填写源码根目录下的浏览器 Python 文件路径，例如 `tools/browsers/chrome.py`、`tools/browsers/edge.py`、`tools/browsers/firefox.py` 或 `tools/browsers/firefox_ech.py`。入口会在启动前检查文件是否存在，并根据模块实际暴露的驱动函数识别浏览器类型；新增浏览器变体不需要再修改中央注册表。

Chrome、Edge、Firefox 及其 Clash/ECH 变体统一使用 `trafficIngestor/traffic_capture_single_csv/action.py`。宿主 profile 通过 `TRAFFIC_ACTION_PROFILE` 传入浏览器模块路径，Clash 代理和诊断通过独立环境变量控制；运行前会把公共 action 同步到 `runtime/workspaces/` 中该 profile 的独立目录。Firefox ECH 任务还会检查密钥日志中的 `ECH_SECRET`、`ECH_CONFIG` 及后续 TLS 1.3 流量密钥标签，缺失时任务失败并在日志中输出各标签计数。
...
- `python trafficIngestor/host_scheduler_clash/single_csv_profiles.py trafficIngestor/single_csv/edge_clash.py`
  Edge + Clash 采集；使用 Edge 镜像、Edge 版本探测和独立 Clash 运行命名空间。
- `python trafficIngestor/host_scheduler_clash/single_csv_profiles.py trafficIngestor/single_csv/firefox_clash.py`
  Firefox + Clash 采集；使用 Firefox 镜像、Firefox 版本探测和独立 Clash 运行命名空间。
- `python trafficIngestor/host_scheduler_clash/single_csv_profiles.py trafficIngestor/single_csv/fixed_ip_europe_clash.py`
  Chrome + Clash 欧洲采集；配置通过 `VPN_INFO_NAME = "vpns_info_europ"` 指定 `configs/clash/sever_info.py` 里的节点数组。
- `TRAFFIC_INGESTOR_RUN_NAME=my_firefox_batch python trafficIngestor/host_scheduler_clash/single_csv_profiles.py trafficIngestor/single_csv/firefox_clash.py`
  可选，用显式运行名覆盖配置文件的默认隔离规则；适合同一配置并行跑多批任务。
- Edge / Firefox Clash 不再维护独立 action 源文件；统一 action 根据配置的浏览器模块选择后端并注入 Clash 代理。各配置仍使用独立运行命名空间和运行目录。

## Trojan 外层解密

- `trafficIngestor/host_scheduler_clash/` 基类会在容器启动 Clash 时导出 `SSLKEYLOGFILE`，默认路径为 `HOST_CODE_PATH/clash_runtime/<container>/trojan_outer_sslkey.log`。
