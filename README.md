...
trafficIngestor_ech/                ECH 宿主机侧调度脚本
...
- `python trafficIngestor/traffic_capture_single_csv.py`
  Chrome 批量流量采集
- `python trafficIngestor_ech/traffic_capture_single_csv_ech.py`
  强制启用 ECH 的 Chrome 批量流量采集
...
### CSV 采集任务
`trafficIngestor/` 与 `trafficIngestor_ech/` 下的 `traffic_capture_single_*.py` 是具体采集器。常改配置包括：

`trafficIngestor/` 与 `trafficIngestor_ech/` 下基于 `BaseTrafficIngestor` 的入口默认按脚本文件名自动推导 `BASE_NAME`、`CONTAINER_PREFIX` 和 `HOST_CODE_PATH`；多数入口只保留 CSV、并发、输出目录等实际业务配置。默认 Chrome 镜像统一来自基类，只有确实需要复用其他代码目录时，才需要显式覆盖 `HOST_CODE_PATH`。
